...
// ID of the div where the tree will be rendered;
      ...
      this.adjustLines(document.getElementById(this.name),true);
		...
		drawNode: function(p_ulElement,p_node) {
			...
			if (p_node.childNodes.length == 0) {
				v_exp_col = createImgElement(null,'exp_col',null);
			}
			else {
				if (p_node.expanded) {
		...
		nodeContextMenu: function(p_event,p_node) {
			if (p_event.button==2) {
				...
				if (p_node.contextMenu!=undefined) {
					...
					v_div.style.left = event.pageX + 'px';
					v_div.style.top = event.pageY + 'px';
